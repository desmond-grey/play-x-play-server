...
function getGameStateForTableId(tableId) {
    const gameId = activeTables.get(tableId);
    return activeGames.get(gameId);
}
...
function gameHasBeenWon(gameState) {
    ...
    const playerTwoHasWon =
        playerTwoScore >= 11
        &&
        playerTwoScore >= 2 + playerOneScore;
...
function swapServeAfterPointScoredIfNecessary(gameState) {
    if (getRawPointsScored(gameState)) {
        // if both players have 10 or above, swap every point
        if (gameState.players[0].pointsScored >= 10 && gameState.players[0].pointsScored >= 10) {
            swapServeForTwoPLayerGame(gameState.players);

        }

        // otherwise, serve swaps every two points (which is to say, when the total points are even)
        else if (getTotalPoints(gameState) % 2 === 0) {
            swapServeForTwoPLayerGame(gameState.players);
        }
    }
}
